...
CLF_SAVE_DIR = Path("/content/drive/MyDrive/fine_tune/Hackathon-Butterfly-Hybrid-Detection-Main/trained_clfs")
DEVICE = "cuda:0"
BATCH_SIZE = 4
...
def prepare_data_loaders(train_data, test_data, img_path):
    train_sig_dset = ButterflyDataset(train_data, img_path, transforms=data_transforms())
    tr_sig_dloader = DataLoader(train_sig_dset, batch_size=BATCH_SIZE, shuffle=False, num_workers=8)
    test_dset = ButterflyDataset(test_data, img_path, transforms=data_transforms())
